...
## 개별 가리기 + 모션/사운드 (2026-03-30)
- [ ] WordCard 개별 가리기/공개 토글 버튼 추가 (전체 가리기 모드와 독립)
- [ ] 카드 뜻 공개 시 플립/슬라이드 애니메이션 (Reanimated)
- [ ] 탭 사운드(가리기) + 공개 사운드 적용 (expo-audio)
- [ ] 전체 가리기 모드 ON 시 모든 카드 일괄 가리기 유지

## 개별 가리기 + 모션/사운드 (2026-03-30)
- [x] "가리기" 버튼명 → "전체 가리기"로 변경
- [x] WordCard 개별 가리기/공개 토글 버튼 추가 (전체 가리기 모드와 독립)
- [x] 카드 뜻 공개 시 슬라이드 다운 + 페이드 인 애니메이션 (Reanimated)
- [x] 탭 사운드(가리기) + 공개 사운드 적용 (expo-audio)
- [x] 전체 가리기 모드 ON 시 모든 카드 일괄 가리기 유지

## 검색 정렬 개선 (2026-03-30)
- [x] score 0(시작 일치) 내에서 입력 철자 위치(index) 기준 정렬 → se로 시작하는 단어가 중간 포함보다 항상 위에 오도록

## 최신 논리기출 S급 추가 (2026-06-30)
- [x] 논토니오 실전17·18강, 심화19강, 실전20강 원문 기출 34문항 exam-questions.ts 추가 (필기 무시·원문 그대로 복습용)
- [x] 형광펜 표시 선지를 정답으로 매핑, 논리완성(logic) 섹션·최우선 암기 대상으로 편입
- [x] 해설에 영영사전 정의 + 동의어쌍 + 관련숙어 + 선지단어 뜻 전부 포함 (답 선택 후 공개)
- [x] schools.ts 논리 시리즈 설명 199문항으로 업데이트
- [x] 전체 302문항 데이터 검증 (정답 인덱스·해설 누락 0건, tsc 0 오류, 테스트 31개 통과)
- [ ] 미수록분: 18강 11·12번(p.116), 20강 9·10번(p.127) — 원본 사진 누락, 추후 추가 필요

## 학습 일정 관리 (2026-06-30)
- [x] store.ts에 일정 CRUD 함수 추가 (load/add/update/toggle/remove/clearDone + 날짜·시간순 정렬)
- [x] 일정 탭 신규 구현 (app/(tabs)/schedule.tsx): 날짜별 그룹·D-day 배지·완료 체크·요약 카드
- [x] 일정 추가/수정 모달 (제목·날짜·시간·메모 입력, 빠른 날짜 칩, 입력 검증)
- [x] 완료 일정 일괄 정리 + 개별 삭제
- [x] 탭 네비게이션에 일정 탭 추가 (calendar 아이콘)
- [x] 일정 store 단위 테스트 8개 추가 (전체 31개 통과)
