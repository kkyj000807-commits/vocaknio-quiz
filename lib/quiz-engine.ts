...
for (const item of VOCAB_WITH_SYNONYMS) {
  for (const detail of getSynonymDetails(item)) {
    ...
    if (!synonymOptionMap.has(key)) synonymOptionMap.set(key, { ...detail, key });
  }
}
...
function buildSynonymChoices(item: VocabItem, includeMeaning: boolean): QuizChoice[] | null {
  const correctDetails = getSynonymDetails(item);
  if (correctDetails.length === 0) return null;

  const correct = correctDetails[Math.floor(Math.random() * correctDetails.length)];
  const related = getRelatedWords(item);
  const targetMeanings = [item.k, item.conceptLabel, ...correctDetails.map((detail) => detail.meaning)].filter(
    Boolean,
  );
  const distractors = pickUnique(
    ...
    (option) => {
      ...
      if (related.has(word) || isAcceptedSynonym(item, option.word)) return false;
      return !targetMeanings.some((meaning) => meaningsOverlap(meaning, option.meaning));
    },
  );
  ...
  return shuffle([
    ...
    ...distractors.map((detail, index) => synonymChoice(detail, false, includeMeaning, index + 1)),
  ]);
}
...
function buildMeaningChoices(item: VocabItem): QuizChoice[] | null {
  ...
  const distractors = pickUnique(
    ...
    (candidate) => {
      ...
      if (!normalizedMeaning || usedMeanings.has(normalizedMeaning)) return false;
      if (related.has(normalizeWord(candidate.w))) return false;
      if (candidate.conceptId && candidate.conceptId === item.conceptId) return false;
...
function makeQuestion(
  ...
  const asksForSynonym =
    mode === "syn-choice" || mode === "syn-kor-choice" || (mode === "kor-choice" && choiceLang === "english");
...
function resolvePool(options: BuildQuizOptions): VocabItem[] {
  if (options.itemNums) {
    return options.itemNums.map(getVocabItem).filter((item): item is VocabItem => Boolean(item));
  }

  const range = RANGES.find((candidate) => candidate.id === options.rangeId);
  if (range?.kind === "idioms") {
    return VOCAB.filter((item) => item.type === "idiom" || item.type === "phrase");
  }
...
export function buildQuizQuestions(options: BuildQuizOptions): QuizQuestion[] {
  ...
  if (options.mode === "syn-choice" || options.mode === "syn-kor-choice" || options.mode === "syn-type") {
    pool = pool.filter((item) => item.s.length > 0);
  }
  if (options.mode === "kor-choice" && choiceLang === "english") {
    pool = pool.filter((item) => item.s.length > 0);
  }

  const questions: QuizQuestion[] = [];
  for (const item of shuffle(pool)) {
    const question = makeQuestion(item, options.mode, choiceLang);
...
export function buildReviewQuestions(itemNums: number[], count: number): QuizQuestion[] {
  const questions: QuizQuestion[] = [];
  const items = shuffle(
    itemNums.map(getVocabItem).filter((item): item is VocabItem => Boolean(item)),
  );
...
export function isChoiceCorrect(question: QuizQuestion, choice: QuizChoice): boolean {
...
export function isTypedAnswerCorrect(question: QuizQuestion, answer: string): boolean {
...
export function validateQuestion(question: QuizQuestion): boolean {
  ...
  if (new Set(question.choices.map((choice) => choice.label)).size !== 4) return false;
  return question.choices.filter((choice) => isChoiceCorrect(question, choice)).length === 1;
}
