...
기준: 2026.09.27 01:29 KST. 이 파일은 단일 현재 상태다. 구조/명령은 DEVELOPMENT.md, 운영 원칙은 AI_WORK_RULES.md, 자동 재개는 AUTOMATION_RUNBOOK.md를 따른다. 과거 todo/대화/자동화의 1.8 후보 문구보다 최신 실제 Git·공개 상태가 우선한다.
...
## 현재 batch — 검수 숙어·법률 개념 동의 표현 35sense backfill PARTIAL VERIFIED / 미배포

- Source main `13023f9`까지 `capitalize on`, `carry out`, `delve into`, `look after`, `put up with`, `at stake`, `to boot`, `by the same token`, `a case in point`, `clamp down on`, `crack down on`, `cut the ground from under someone`, `put A at ease`, `on the verge of`, `pull the rug from under someone`, `release - from house arrest`, `There is more than one way to skin a cat`, `take the law into one's own hands`, `throw good money after bad`, `give someone the benefit of the doubt`, `new normal`, `executive privilege`, `jury nullification`, `run into`, `on behalf of`, `get over` 2sense, `come to terms with` 2sense, `in the face of`, `in the wake of`, `make for`, `account for` 3sense의 공란을 독립 사전·thesaurus 본문으로 다시 검수했다. 공개 Pages는 계속 3.1이며 이 미완료 backfill batch는 배포하지 않았다.
...
- `executive privilege`는 행정부 기밀 소통을 공개하지 않을 권한이라는 `authority to withhold confidential executive communications`, `jury nullification`은 배심이 법 위반을 인정하면서도 그 법 적용을 의도적으로 거부한다는 `a jury's deliberate refusal to apply the law`를 exact 설명형 paraphrase로 연결했다. 일반적인 특권이나 증거 부족에 따른 무죄 평결과 합치지 않았다.
- `get over`는 질병·충격 뒤 회복(`recover from`)과 문제·장애 극복(`overcome`)을, `come to terms with`는 불편한 현실 수용(`come to accept`)과 상대방과의 합의(`reach an agreement`)를 각각 독립 sense로 분리했다. 기존 `:primary` 혼합 항목은 canonical 산출물에서 제외하고 원천 occurrence와 사용자 기록은 그대로 보존했다.
- `account for`는 원인 자체가 결과를 설명함(`explain`/`be responsible for`), 사람이 이유를 설명함(`explain`), 전체 중 비중을 이룸(`constitute`/`make up`)의 3sense로 분리했다. `in the face of`는 실제 occurrence의 양보 sense만 `despite`/`in spite of`에, `in the wake of`는 사건 뒤 여파 sense만 `in the aftermath of`에, `make for`는 결과 기여 sense만 `lead to`/`contribute to`에 연결했다. 사전에는 있으나 현행 occurrence에 없는 다른 sense는 합치지 않았다.
- `run into`의 실제 원본 뜻은 사람을 우연히 만나다이므로 충돌·곤란·금액 sense를 한 묶음에서 제거하고 `bump into`/`run across`를 exact로 연결했다. `on behalf of`도 실제 원본의 대표·대신 sense로 좁혀 `as a representative of`를 exact 설명형 paraphrase로 연결했으며, 이익을 위한 별도 sense는 합치지 않았다.
...
- 정제 후 canonical denominator는101sense다. 필드 공란은 영영정의0/한국어0/문맥0/예문0/예문해설0/구별메모0/출처0, 동의어51→20이다. 상태는 COMPLETE46→81 / NEEDS_REVIEW51→20 / VERIFIED_NO_DATA0 / FAILED0 / PENDING0이다. 원본38,163행/고유표제어13,347개, canonical 연결230행/미매핑37,933행은 변하지 않았다.
...
- VERIFIED: site vocab→learning/canonical backfill을 실제 재실행해 asset→public JSON까지 동일하게 갱신했다. targeted 2파일15테스트, 전체 TypeScript·데이터 감사·25파일198테스트 통과(인증1skip), 변경 lint 오류0(기존 module-format 경고만 존재). 서른다섯 sense의 COMPLETE 상태·exact/near 분리·공란 없음과 배포용 데이터 일치를 회귀 검사했다.
- NOT DONE: 전체38,163행 canonical sense 이관과 필수 필드 보강은 완료가 아니다. 동의어20sense와 미매핑37,933행이 남아 있어 전체 완료/coverage100%라고 하지 않는다. 공개 Pages 배포와 실제 Safari/Chrome/Samsung Internet 검증도 하지 않았다.
- NEXT/P0: NEEDS_REVIEW20개를 숙어 우선으로 다른 허가·검증 출처와 sense 경계에 따라 검수하고, 실제 부재가 확인된 경우에만 VERIFIED_NO_DATA로 전환한다. 미매핑37,933행은 숙어·오답·고빈도 순으로 안전한 canonical mapping batch를 만든다. 새 UI/영영 문제 섹션은 공란 작업보다 앞서지 않는다.
...
- 깊이 학습152sense/218행, 문맥 연습6표현/13문항/19행, 구성요소 해설15표현, 새 본문풀6문항/16행, active-recall11sense/14행은 서로 다른 지표다.
