...
기준: 2026.09.26 00:55 KST. 이 파일은 단일 현재 상태다. 구조/명령은 DEVELOPMENT.md, 운영 원칙은 AI_WORK_RULES.md, 자동 재개는 AUTOMATION_RUNBOOK.md를 따른다. 과거 todo/대화/자동화의 1.8 후보 문구보다 최신 실제 Git·공개 상태가 우선한다.
...
- Source 구현 `bde7aee`는 로컬 main에서 검증 완료했다. 공개 Pages는 계속 3.1이며 이 미완료 backfill batch는 배포하지 않았다.
...
- VERIFIED: 전체 TypeScript·데이터 감사·25파일194테스트 통과(인증1skip), 신규 canonical 테스트4개와 source→public 동의어 전달 회귀, 변경 lint 오류0(기존 Array 스타일 경고2), Production3.1 로컬 build audit HTML22/학습JSON8/참조누락0/루트자산0. 로컬 production in-app browser에서 `abide by` 1sense와 `all but` 2sense의 영영정의/한국어/문맥/예문/판별단서가 실제로 펼쳐지는 것을 확인했다. 이번 두 동의어의 새 브라우저 수동 확인과 새 production build는 미완료 batch라 반복하지 않았다.
