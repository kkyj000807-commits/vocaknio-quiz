...
- 버전: 1.6 (직전 공개 1.5)
- 앱 표시: 버전 1.6 · 최근 수정 2026.09.05 12:27 KST · Production 기준
- 공개 URL: https://kkyj000807-commits.github.io/vocaknio-quiz/
- 소스: db18a65 / Pages: 12e029c5ae4f00cb00cc806c9c77d25192f4ee1f
- Pages Actions 33941979234: 2026.09.05 12:30:34 KST 성공 확인.
- 공개 HTTP 검증: 2026.09.05 12:31 KST, 홈·설정·단어장·문제 풀이 HTTP 200, 모두 새 번들 참조.
- 번들: entry-6dc3aefe0154e0516ba52ce7d3dc033c.js
- 공개/로컬 번들 SHA256 일치: 7d37e130250f6c2fa3a0e3bf64f7b469bbd6b8915a6f0fa73e9395bba0202cac
- 1.6 학습 JSON 8개: HTTP 200, 버전 및 전체 JSON 내용이 빌드본과 일치.
- 기존 해시 자산과 1.4·1.5 학습 파일은 보존했다. 새 학습 데이터는 /data/vocab-learning/1.6/에 있다.
...
- 43개 중 기본 뜻 문구 수정은 41개이며 in spite of A 2개는 올바른 기본 뜻을 유지하고 해설·예문만 추가했다.
- 추가 표현: all but, a case in point, put up with, in the wake of, look after, run into, take for granted, get over, carry out, make for, by no means, on behalf of, account for, come to terms with, on the verge of, in spite of, at stake.
...
- 기존 WordNet 원문 40개 학습 엔트리 + 보완 63개 행별 엔트리 = 103개 엔트리, 182개 목록 연결. 이는 서로 다른 숙어 103개가 검수됐다는 뜻이 아니다.
...
- 전용 TypeScript: 통과.
- 관련 Vitest: 7파일 54테스트 통과. 명령에 --dir tests를 넣어 사용자 tmp/pydeps2의 OneDrive 접근 오류를 피한다.
...
- 공개 인앱 Chromium: 캐시 삭제나 강력 새로고침 없이 1.6 표시 확인. 핵심 숙어 첫 칸, 단어장 첫 칸/기본 선택/3,731개, 숙어 10문제 진입, 정답→다음→이전 시 정답 1 유지 확인.
- 공개 단어장 on the verge of: 보완 뜻, 영영 풀이, 한영 예문 펼침과 미색 화면·세로 스크롤 확인.
- 실제 Windows Chrome 연결은 제공되지 않았고, 실제 iOS Safari도 접근할 수 없다. 인앱 Chromium 결과를 Chrome·Safari 확인으로 대체하지 않는다.
...
- 버전은 release.config.json 한곳, Production 성공 시 +0.1. 다음 후보는 1.7.
